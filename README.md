...
Virtuelles Enviroment erstellen und aktivieren:
````shell
...
Beim Starten wird geprüft, ob das Dataset bereits im Verzeichnis ist, wenn nicht, wird dieses heruntergeladen.
Ansonsten wird das Dataset aus dem Verzeichnis `./data/dataset<Dataset_name_>` geladen. Es muss also kein Datensatz manuell geladen werden.
...
python models.py
````
Zum Schluss folgt noch die `train.py` mit dem Training und der Auswertung der Daten.
